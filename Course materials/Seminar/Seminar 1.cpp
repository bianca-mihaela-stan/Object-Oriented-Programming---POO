...
/*
int  gv(int ii, int *pii,int &rii)    // functie care intoarce  un intreg - prin valoare 
{int vl;
...
*/
/*
int * gp(int ii,int *pii, int & rii) // functie care intoarce adresa unui intreg -prin valoare
{int vl;
...
/*
int & gr(int ii, int *pii,int &rii)    // functie care intoarce un alt nume pentru un intreg -prin referinta
{int vl;
...
int i,j;
const int * cp; //**** pointer care protejeaza zona 
cp=&i;
i=1;
// *cp=1;  se poate modifica i dar nu prin pointerul cp

const int & cr=i; //**** referinta care protejeaza zona
i=1;
//  cr=2; se poate modifica i dar nu prin cr


int * const pc=&i;//****pointer constant- necesar initializarea la declarare
*pc=1; // pot modifica zona
//  pc=&j; //nu poate sa adresa alta zona

int v[10]; // v este pointer constant
*v=1; // echivalent cu v[0]=1;
//  v=&j; nu poate sa adreseze alta zona

//  fRef(f());  //nu -referinta ar putea modifica zona -temporarul nu se poate modifica
//   fRef(2); //nu-referinta ar putea modifica zona -constanta nu se poate modifica
 fConstRef(f());
 fConstRef(2);
}
*/
